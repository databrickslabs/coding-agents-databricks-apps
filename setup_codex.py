...
# Resolve a fresh credential per Codex process/request. This mirrors ucode's
# auth command and avoids persisting a short-lived broker bearer in ~/.codex.
helper_path = write_token_helper(codex_dir)
helper_python = os.environ.get("CODA_VENV_PYTHON") or sys.executable or "python3"
auth_config = (
    f"auth = {{ command = {json.dumps(helper_python)}, "
    f"args = [{json.dumps(str(helper_path))}], "
    "timeout_ms = 5000, refresh_interval_ms = 900000 }"
)
# Remove the static token file generated by older CoDA versions, but only when
# its marker proves that CoDA owns it.
legacy_env_path = codex_dir / ".env"
if legacy_env_path.exists() and legacy_env_path.read_text().startswith(
    "# Databricks token for Codex CLI"
):
...
# 4. Codex invokes the helper configured above whenever it needs a bearer.
# No token is written to a terminal-visible .env file.
...
if env_lines:
    ...
    print(f"Codex CLI tracing env configured: {env_path}")
