...
Zerobus writes **work fine in the `<dev-profile>` dev workspace** (so the whole
...
> **Dev vs prod — how to build this.** Because Zerobus **works on `<dev-profile>`**,
...
> <dev-profile> for a green run in prod on the write path.
>
...
<dev-profile> instance with the restricted-network flag on**:
