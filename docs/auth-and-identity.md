...
|---|---|---|
| **The user** (e.g. `<owner-email>`) via the PAT in `~/.databrickscfg [DEFAULT]` | A **user** personal access token (`dapiâ¦`), kept fresh by the PAT rotator | **Everything the terminal / CLI does**: `databricks` commands, `git`, `gh`, workspace/UC ops, file writes. Also the token the content-filter proxy injects for OpenCode / Hermes / Codex model calls. |
