...
- `private` → `dgokeeffe/coding-agents-databricks-apps-private` (where the app
...
- **`DEFAULT` profile PAT goes stale.** Named profiles (`daveok`, `lakemeter`,
...
- Personal Omnigent server app `omnigents-daveok` may be STOPPED (reversible with
  `databricks apps start omnigents-daveok --profile daveok`).
